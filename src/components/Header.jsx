...
import { useState, useRef } from "react";
import { Link } from "react-router-dom";
import { Search, User, ShoppingBag, ReceiptText, X } from "lucide-react";
...
export default function Header() {
  ...
  return (
    <header className="relative w-full bg-white z-[100] border-b border-gray-100">
      {/* 1. Top Utility Bar */}
      <div className="bg-gray-50 border-b border-gray-100 flex items-center h-[32px]">
        <div className="max-w-[1200px] mx-auto w-full flex justify-end px-6 items-center gap-6">
          {[
            { text: "로그인", to: "/login" },
            { text: "회원가입", to: "/signup" },
            ...
            <Link
              key={item.text}
              to={item.to}
              className="text-[11px] font-medium text-gray-400 hover:text-[#3ea76e] transition-colors"
            >
              {item.text}
            </Link>
          ))}
      ...
      <div className="max-w-[1200px] mx-auto flex items-center justify-between px-6 h-[100px]">
        ...
        <div className="flex-shrink-0">
          <Link to="/" className="inline-flex items-center">
            <div className="text-[#3ea76e] text-[38px] font-black tracking-[-0.1em] leading-none">
              SWIFFY
              <span className="text-[14px] align-top ml-0.5 opacity-70 italic">
                ®
              </span>
            </div>
          </Link>
        ...
        {/* Search Bar - Fixed text color & background */}
        <div className="flex-1 max-w-[500px] mx-10 relative">
          <div
            className={`flex items-center rounded-full px-6 py-2.5 transition-all bg-gray-50 border ${
              isSearchFocus
                ? "ring-2 ring-[#3ea76e] border-transparent bg-white shadow-lg"
                : "border-gray-100"
            }`}
          >
            <input
              type="text"
              placeholder="우리 아이가 좋아하는 간식 검색"
              value={searchValue}
              onChange={(e) => setSearchValue(e.target.value)}
              onFocus={() => setIsSearchFocus(true)}
              className="flex-1 outline-none text-[15px] font-medium text-neutral-800 placeholder:text-gray-300 bg-transparent py-1"
            />
            {searchValue && (
              <X
                size={18}
                className="text-gray-300 cursor-pointer hover:text-gray-500 mr-2"
                onClick={() => setSearchValue("")}
              />
            )}
            <button className="text-[#3ea76e] hover:scale-110 transition-transform cursor-pointer">
              <Search size={22} strokeWidth={2.5} />
            </button>
          </div>
          ...
          {isSearchFocus && (
            <>
              ...
              />
              <div className="absolute top-[calc(100%+12px)] left-0 w-full bg-white shadow-2xl px-7 py-8 z-[200] border border-gray-100 rounded-[24px]">
                <p className="text-[14px] font-bold text-neutral-800 mb-5 flex items-center gap-2">
                  <span className="w-1.5 h-1.5 bg-[#3ea76e] rounded-full"></span>
                  인기검색어
                </p>
                <div
                ...
                >
                  {popularSearches.map((tag) => (
                    <button
                      ...
                      className="whitespace-nowrap px-5 py-2 bg-gray-50 rounded-full text-[13px] font-semibold text-gray-600 hover:bg-[#3ea76e] hover:text-white transition-all"
                    >
                      {tag}
                    </button>
                  ))}
        ...
        {/* Icons Menu */}
        <div className="flex-shrink-0 flex justify-end gap-8">
          {[
            {
              label: "마이페이지",
              icon: <User size={26} strokeWidth={1.8} />,
              to: "/profile",
            },
            {
              label: "주문조회",
              icon: <ReceiptText size={26} strokeWidth={1.8} />,
              to: "/order/list",
            },
            {
              label: "장바구니",
            ...
            >
              <div className="relative text-neutral-800 group-hover:text-[#3ea76e] transition-all group-hover:-translate-y-0.5">
                {item.icon}
                {item.badge && (
                  <span className="absolute -top-1 -right-1.5 bg-[#ff5c5c] text-white text-[9px] font-bold w-[17px] h-[17px] flex items-center justify-center rounded-full ring-2 ring-white">
                    {item.badge}
              ...
              <span className="text-[11px] font-bold mt-1.5 text-gray-400 group-hover:text-[#3ea76e] transition-colors">
                {item.label}
              </span>
            </Link>
          ))}
      ...
      {/* 3. Navigation Bar */}
      <nav className="bg-white border-t border-gray-100 h-[54px]">
        <div className="max-w-[1200px] mx-auto flex items-center justify-center gap-14 h-full">
          {navItems.map((item) => (
            <Link
              key={item.label}
              to={item.to}
              className="group relative py-1 h-full flex items-center"
            >
              <span className="text-[16px] font-bold text-neutral-800 transition-colors group-hover:text-[#3ea76e]">
                {item.label}
              </span>
              <span className="absolute bottom-0 left-0 w-0 h-0.5 bg-[#3ea76e] transition-all group-hover:w-full"></span>
            </Link>
